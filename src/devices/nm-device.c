...
dhcp4_grace_period_expired (gpointer user_data)
{
	...
	_LOGE (LOGD_DHCP4, "DHCPv4: grace period expired");
...
dhcp4_fail (NMDevice *self, gboolean timeout)
{
	...
	if (   !priv->dhcp4.was_active
		...
		_LOGD (LOGD_DHCP4, "DHCPv4: failed for timeout or during initial setup");
...
dhcp6_grace_period_expired (gpointer user_data)
{
	...
	_LOGE (LOGD_DHCP6, "DHCPv6: grace period expired");
...
dhcp6_fail (NMDevice *self, gboolean timeout)
{
	...
	if (is_dhcp_managed) {
		...
		if (   !priv->dhcp6.was_active
			...
			_LOGD (LOGD_DHCP4, "DHCPv6: failed for timeout or during initial setup");
